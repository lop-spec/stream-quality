...
[手机网页](https://lop-spec.github.io/stream-quality/) · 默认只测固定 SSE 网络质量，全节点并行；有界下载仅显式启用。**不调用模型，不读取模型账户，不把 Mbps 换算成 tok/s。**

> 开发候选：179 路本地并行已完整完成，实测 20.55 秒；严格“点击后≤20秒”未达标。保持原有401段/20秒规约与校验，不以截断样本冒充成功。真实订阅线路的并发准确性尚未验收，未据此更新生产端点或日常 EXE。
...
# 仅显式需要时增加旧版 8 MiB 下载（多节点保持串行）
node stream-quality-runner.cjs --endpoint https://stream-quality.1781297309.workers.dev --download
...
| 复测 | 默认1次初筛，全节点同时启动；可选3个先后窗口，每轮全并行，3/3成功才 verified。显式下载模式仍串行 |
| 流量 | 默认每节点每轮 SSE 负载上限105 KiB，无下载/上传测速；不含握手/协议开销 |
| 时间边界 | 20秒是完整采样窗口，不是端到端时限；还需核心准备、连接和收尾，三轮至少60秒 |
| 比较边界 | 新成绩 measurement=sse-only，profileKey另加sse-only后缀；不混排旧完整下载工况 |

179路仅SSE净负载约7.33 Mbps。全并发共享本机、接入网及机场带宽，不能把它等同于各节点独占线路的表现；本机事件循环停顿超过100ms会将本轮标记为测量无效而非节点故障，该检查不能排除共享链路争抢。
...
- 候选源码每隔离实例最多256路SSE（每IP每分钟1024次），下载单独限制8路/每IP每分钟40次、固定响应体。线上旧端点更新前仍受原8路上限约束。此内存限制不是全球硬配额或 DDoS 防护；免费 Worker 也有请求/CPU额度，不能承诺永久无限流量或 SLA。源过载会明确返回429/错误，不能作为节点成绩。
- 免费档 CPU 是否适合该地区/负载需看部署实测；不自动升级付费计划。勿向不可信测试端点发送任何凭据。

## 验收

`npm test` 包含严格帧大小/序号、断流、源抖动识别、网络停顿和攒包、179路并行、3轮独立窗口、取消真实 socket、源端独立限额、完整20秒本地代理链路与显式8MiB下载。`SQ_REQUIRE_20S=1 node --test tests/parallel.test.cjs` 对原始≤20000ms目标做严格断言，当前预期失败，普通回归通过不等于此目标通过。真实端点需额外执行 CLI/网页实测，构建通过不等于网络可用。
