  ...
  try {
    for (let i = 0; i < count; i++) {
      const result = await SQ.browserProbe(base, { signal: controller.signal,
        onProgress: phase => { $('status').textContent = `第 ${i + 1}/${count} 轮 · ${phase === 'stream' ? '流式采样 20 秒' : '串行下载 8 MiB'}`; } });
    ...
    const cards = [['流式承载', pass ? '达标' : '有波动'], ['样本吞吐', `${f(median(r => r.download.mbps))} Mbps`],
    ...
    $('details').textContent = `${results[0].endpoint} · ${results[0].location} · ${count === 3 ? '三轮完整复测' : '仅单次初筛'}。${results.some(r => r.download.shortSample) ? '下载不足 1 秒，短样本波动大，不能视为带宽上限。' : ''} 不等于真实模型速度。`;
    $('results').hidden = false; $('status').textContent = '测速完成；流式达标与下载速度分别展示。';
